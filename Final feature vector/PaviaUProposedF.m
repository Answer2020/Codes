%This code implements our proposed method using our prposed final feature
%vector
accArray=zeros(1,10);
...
%OATestMean=zeros(4,10);
%OATestStd=zeros(4,10);
%KappaTestMean=zeros(4,10);
%KappaTestStd=zeros(4,10);
%AATestMean=zeros(4,10);
%AATestStd=zeros(4,10);

%OATestMeanEachclass=zeros(4,10,numClasses);
%OATestStdEachclass=zeros(4,10,numClasses);
%%
...
for iterCounter=1:10
    ...
    %test pixels.
    [xTrainImages, tTrain,ijTrain,trainLabels,TestData, tTest,ijTest,testlabels,row,col]=buildfeatures(neighbourCount,PCAnumComp,TotTrainingData, TotTrainingDataLabels,ijindex,trainInd,testInd);
    ...
    figure, imshow(RGB)
    name=sprintf('CMapPaviaUDiatanceEMAP%d',iterCounter)
