function [trainData,tTrain,ijTrain,trainLabels,TestData, tTest,ijTest,testlabels,r,c]=buildfeatures(neighbourSize,numComp,MagThreshold,P,sigma,Data,labels,ijindex,trainInd,testInd)
...
magImage=GradientImage(paviaU,sigma);
...
%%
%Calculating the distance transform image using Euclidean distance
T=bwdist(mask);
%figure(6), imshow(T, []);
mi=min(T(:));
minimum = ones(r,c)*mi;
...
for k1=1:size(trainInd,1)
    ...
    for ii=ijindex(trainInd(k1,1))-x:ijindex(trainInd(k1,1))+x
        for jj=ijindex(trainInd(k1,1),2)-x:ijindex(trainInd(k1,1),2)+x
            ...
            temp=[reshape(pddedPaviaPCA(ii+x,jj+x,:),[1,numComp]) distance];
